...
class _VirtualAccountState extends State<VirtualAccount> {
  ...
  _updateTimer() {
    ...
    setState(() {
      timesLeft = "${twoDigits(hours)}:${twoDigits(minutes)}:${twoDigits(seconds)}";
    });
    ...
    return WillPopScope(
      onWillPop: () async {
        final shouldPop = await showDialog<bool>(
          ...
          builder: (context) {
            return AlertDialog(
              title: const Text('Jika anda kembali, maka anda akan membatalkan transaksi'),
                    ...
                    child: GestureDetector(
                      ...
                      onTap: () {
                        _copyTextToClipboard(
                            context, widget.data.midtrans.vaNumbers.first.vaNumber);
            ...
            return BlocListener<CekTransaksiBloc, CekTransaksiState>(
              listener: (context, state) {
                ...
                if (state is CekTransaksiLoaded) {
                  if (state.data.userTransaction.transactionStatus == "done") {
                    Navigator.pushAndRemoveUntil(
                        context,
                        MaterialPageRoute(
                          builder: (context) => StatusBerhasil(),
                        ),
                        (_) => false);
                  }
                  if (state.data.userTransaction.transactionStatus == "pending") {
                    ScaffoldMessenger.of(context)
                        .showSnackBar(SnackBar(content: Text("Selesaikan pembayaran anda")));
                  }
                }
              },
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Button(
                      onTap: () {
                        print(topupState);
                        context.read<CekTransaksiBloc>().add(CekTransaksiInisiate(
